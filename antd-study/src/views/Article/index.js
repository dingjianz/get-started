...
export default class ArticleList extends Component {
  ...
  setData = (state) => { // 如果请求完成之后组件已经销毁，就不需要再setState
  ...
  createColumns = (columnsKeys) => {
    const columns =  columnsKeys.map(item => {
      if (item === 'amount') {
        ...
        return {
          ...
          render: (text,reacord) => {
            return (
              moment(reacord.createAt).format('YYYY年MM月DD日 hh:mm:ss')
            )
          }
        }
      }
    ...
    columns.push({
      ...
      render: (text, record) => {
        return (
          <Button.Group>
            <Button size="small" type="primary" onClick={() => this.toEditPage(record)}>编辑</Button>
  ...
  render() {
    const { dataSource, columns, total, isOpen, isLoading, pageSize, pageNo, deleteModalFlag, deleteArticleTitle, deleteConfirmLoading } = this.state
    return (
      <>
        <Card
          title="文章列表"
        ...
        ><Paragraph>{deleteArticleTitle}</Paragraph></Modal>
      </>
    )
  }
}