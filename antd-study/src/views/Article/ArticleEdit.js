...
class ArticleEdit extends Component {
  ...
  getArticleDetailById = () => { // 获取文章详情
    ajax.post('/api/v1/article/' + this.state.articleId).then(res => {
  ...
  editArticleById = ( params ) => { // 修改文章
    this.setState({
      spinFlag: true
    })
    ajax.post('/api/v1/articleEdit/'+this.state.articleId, params).then(res => {
    ...
    return (
      <Spin spinning={spinFlag}>
        <Card
          title="文章编辑"
          bordered={false}
          extra={<Button onClick={this.goBack}>返回</Button>}
        >
          <Form
            {...formItemLayout}
